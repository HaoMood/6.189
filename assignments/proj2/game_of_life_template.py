...
class Board(object):
    ...
    def get_block_neighbors(self, block):
        ...
        if x == 0:  # Top row.
            ...
            if y == 0:  # Left col.
                ...
                neighbors.remove((x + 1, y - 1))
            if y == self.width - 1: # Right col.
                neighbors.remove((x, y + 1))
                neighbors.remove((x + 1, y + 1))
        elif x == self.height - 1:    # Bottom row.
            neighbors.remove((x + 1, y - 1))
            neighbors.remove((x + 1, y))
            neighbors.remove((x + 1, y + 1))
            if y == 0:  # Left col.
            ...
            if y == self.width - 1: # Right col.
                neighbors.remove((x - 1, y + 1))
                neighbors.remove((x, y + 1))
        else:   # Not in top and bottom row.
            if y == 0:  # Left col.
                neighbors.remove((x - 1, y - 1))
                neighbors.remove((x, y - 1))
                neighbors.remove((x + 1, y - 1))
        ...
        for pos in neighbors:
            neighbor_blocks.append(self.block_list[pos])
    ...
    def simulate(self):
        ...
        #### YOUR CODE HERE #####
        for pos in self.block_list.values():
            neighbors = self.get_block_neighbors(pos)
            num_neighbors_live = 0
            for nei in neighbors:
                if nei.is_live():
                    num_neighbors_live += 1
            if pos.is_live():
                if num_neighbors_live < 2 or num_neighbors_live > 3:
                    pos.new_status = 'dead'
            else:
                if num_neighbors_live == 3:
                    pos.new_status = 'live'

        for pos in self.block_list.values():
            pos.reset_status(self.canvas)
