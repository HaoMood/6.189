...
class Shape():
    ...
    def can_rotate(self, board):
        ...
        #YOUR CODE HERE
        di = self.rotation_dir
        for block in self.blocks:
            x = self.center_block.x - di * self.center_block.y + di * block.y
            y = self.center_block.y + di * self.center_block.x - di * block.x
...
class Board():
    ...
    def game_over(self):
        ...
        x = self.width / 2
        y = 0
        px = x * Block.BLOCK_SIZE + Block.OUTLINE_WIDTH + Block.BLOCK_SIZE / 2
        py = y * Block.BLOCK_SIZE + Block.OUTLINE_WIDTH + Block.BLOCK_SIZE / 2
        text = Text(Point(px, py), 'Game Over !!!')
...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        #YOUR CODE HERE
        shape_class = self.SHAPES[random.randint(0, len(self.SHAPES) - 1)]
        shape = shape_class(Point(int(self.BOARD_WIDTH / 2), 0))
        return shape
